                    ...
                    <div class="card-body">
                        ...
                        <form class="form" novalidate>
                            ...
                            <div class="form-actions text-right">
                                ...
                                <button type="submit" class="btn btn-sm btn-warning btn-primary">
...
<script type="text/javascript">
	$(document).ready(function() {
        ...
        let editData = () => {
            ...
            if ($('#method').val() === "Edit" && $('#hidden_id').val() != "") {
                $.ajax({url: url,type: "POST",data: {id: id,csrf_token_name: $('input[name=csrf_token_name]').val()},dataType: "JSON",
                    success: function(data) {
                        // console.log(data);
                        $('input[name=csrf_token_name]').val(data.csrf_token_name);
                        $('#usernameUserForm').val(data.username);
                        $('#namaUserForm').val(data.nama);
                        if( data.email.includes('@admin.com')  ){
                            $('#roleUserForm').val('admin').trigger('change');
                            $('#emailAdminUserForm').val(data.email.replace('@admin.com', ''));
                            $('#emailCustomerUserForm').val(data.email);
                        } else {
                            $('#roleUserForm').val('customer').trigger('change');
                            $('#emailCustomerUserForm').val(data.email);
                            $('#emailAdminUserForm').val(data.email.split('@')[0]);
                        }
                        $('#no_telpUserForm').val(data.no_telp);
                        $('#alamatUserForm').val(data.alamat);
